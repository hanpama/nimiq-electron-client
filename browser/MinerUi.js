class MinerUi {
    ...
    _selectMode() {
        ...
        switch (this._mode) {
            case 'solo':
                miner = IPCMiner.createMinerIPCProxy(this.$.blockchain, this.$.accounts, this.$.mempool, this.$.network.time, /*address*/ null);
                break;
            case 'smart': {
                const deviceId = Nimiq.BasePoolMiner.generateDeviceId(this.$.network.config);
                miner = IPCMiner.createSmartPoolMinerIPCProxy(this.$.blockchain, this.$.accounts, this.$.mempool, this.$.network.time, /*address*/ null, deviceId);
                break;
            }
            // case 'nano': {
            //     const deviceId = Nimiq.BasePoolMiner.generateDeviceId(this.$.network.config);
